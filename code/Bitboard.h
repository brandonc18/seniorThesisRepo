...
class Bitboard {
  public:
	...
	void set_bit(int square) { data |= (1ULL << square); }

	// Clear a bit at square index
	void clear_bit(int square) { data &= ~(1ULL << square); }

	// Get bit value at square index
	bool get_bit(int square) { return (data & (1ULL << square)) != 0; }
	...
	// Count set bits
	int count() { return __builtin_popcountll(data); }

	// Overload bitwise operators for combining bitboards
	Bitboard operator|(const Bitboard &other) const { return Bitboard(data | other.data); }
